...
def set_time_axes(nas, dates,freq):
    """
    ...
    for i in range(len(dates)):
        date_list.append([pd.to_datetime(dates[i]),pd.to_datetime(dates[i])+pd.Timedelta(freq)])

    nas.sample_times = date_list
...
def set_variables(nas,data_table,flag_table,headers,units,ss,cols):
    """
...
def ebas_genfile(path,data,flag,date,headers,ss, cols,units,freq):
    """
